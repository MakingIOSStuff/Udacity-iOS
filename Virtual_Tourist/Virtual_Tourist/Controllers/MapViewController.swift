...
class MapViewController: UIViewController, NSFetchedResultsControllerDelegate, MKMapViewDelegate {
    ...
    func savePinToStore(FlickrResponse: [FlickrPhoto]) {
        ...
        print("\(FlickrResponse) was saved to pics")
        let imageLinks = FlickrResponse.map { FlickrPhoto -> ImageURL in
        ...
        pin.addToLinks(NSSet(array: imageLinks))
        try? dataController.viewContext.save()
        self.pin = pin
    }
    
    func mapView(_ mapView: MKMapView, didSelect view: MKAnnotationView) {
        print("time to try to fetch")
        ...
        for pin in fetchedResultsController.fetchedObjects! {
            if pin.latitude == view.annotation?.coordinate.latitude &&
                ...
                print("\(pin.pics!.count) photos saved for \(pin)")
            }
        }
        performSegue(withIdentifier: "presentPhotoGalleryView", sender: self)
    }
